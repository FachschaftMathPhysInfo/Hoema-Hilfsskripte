...
\begin{axiom}[Zermelo-Fraenkel-Axiome] %[vgl Axiom 2.1]
    ...
    \begin{enumerate}[label=(ZF\arabic*)]
        \item \underline{Extensionalitätsaxiom}: Eine Menge wird vollständig durch ihre Elemente bestimmt. Formal, für je zwei Mengen $A$, $B$ gilt $(x \in A \iff x \in B) \implies A = B$.
        
        \emph{Das Extensionalitätsaxiom impliziert unter anderem, dass weitere im folgenden axiomatisierten Mengen eindeutig sind.}
        \item \underline{Paarmengenaxiom}: Wenn $A$, $B$ Mengen sind, dann ist $\{A, B\}$ eine Menge.
        \item \underline{Aussonderungsaxiom}: Wenn $A$ eine Menge ist, und $P(x)$ eine Eigenschaft\footnote{Logisch streng genommen ist $P$ ein \emph{Prädikat}.}, die Elemente $x$ von $A$ entweder erfüllen oder nicht erfüllen können, dann ist die Teilmenge von $A$ definiert durch alle Elemente, die $P(x)$ erfüllen, $\{x \in A | P(x)\}$, eine Menge.
        
        \emph{Die Notation $x \in A | P(x)$, sprich "x in A, für die gilt: P(x)", ist logisch definiert durch $(x \in A) \land P(x)$, aber der senkrechte Strich trennt optisch die Elemente der Menge von den Bedingungen, die diese erfüllen. Manche Autoren schreiben auch $x \in A : P(x)$.}
        ...
        \item \underline{Leermengenaxiom}: Es existiert eine Menge, die keine Elemente enthält. Diese nennen wir die \underline{leere Menge} und schreiben $\e$ oder $\{\}$.
        
        Formal ist die Aussage $\exists \e: \forall x: x \notin \e$.

        Eine Menge $A \neq \e$ nennen wir \underline{nichtleer}.
        \item \underline{Potenzmengenaxiom}: Für jede Menge $A$ existiert eine Menge, die genau alle Teilmengen von $A$ enthält. Diese nennen wir die Potenzmenge von $A$ und schreiben $\Pot(A)$.
        
        Mit (ZF3) gilt $\Pot(A) = \{B | B \subset A\}$.
        ...
        \emph{Man bemerke, dass mit $A \arr A \cup \{A\} \in B$ die Voraussetzung für das Unendlichkeitsaxiom wieder erfüllt ist, damit gilt also auch $A \cup \{A\} \cup \{A \cup \{A\}\} \in B$. Diese Kette lässt sich \underline{induktiv}, d.h. durch wiederholtes Ausführen desselben Argumentes, beliebig weit fortsetzen. $B$ enthält also, sofern es mindestens ein $A$ enthält, gleich unendlich viele Mengen. Daher auch der Name.}
...
\begin{remark}
    \begin{itemize}
        \item (ZF1): Eine gängige Beweismethode zu zeigen, dass zwei Mengen $A$ und $B$ gleich sind, ist, die beiden Implikationen $x \in A \implies x \in B$ und $x \in B \implies x \in A$ zu zeigen. Aus (ZF1) folgt $A = B$.
        \item (ZF3): Das historisch zuerst entstandene \underline{Abstraktionsprinzip}, dass $\{x | P(x)\}$ für alle $P(x)$ eine Menge ist, ist sehr verlockend\footnotemark, aber scheitert an einem logischen Widerspruch, genannt die \underline{Antinomie von Russell}:

        Man setze $\A = \{x | x \notin x\}$, und betrachtet den Fall $x = A$.\\
        Wenn $A \in A$, dann folgt $A \notin A$. Aber wenn $A \notin A$, dann folgt $A \in A$. \absurdum

        In ZF-Mengenlehre ist dieser Widerspruch gleich doppelt ausgeschlossen, mit (ZF3) dadurch, dass $P(x)$ auf eine zugrundeliegende Menge $A$ eingeschränkt wird, und durch (ZF9) dadurch, dass Mengen sich nicht selbst enthalten können (siehe unten).

        Es gibt trotzdem Wege, das Abstraktionsprinzip widerspruchsfrei zu machen, z.B. in der Klassenlogik, in der man den Begriff von Mengen zu \underline{Klassen} erweitert, und Klassen von unterschiedlichen "Ebenen" (genannt \underline{Typen}) unterscheidet, die nicht vermischt werden können. Für die Standardmathematik ist allerdings ZF-Mengenlehre ausreichend.
        \item (ZF5): Die leere Menge ist die einzige explizite Menge, deren Existenz axiomatisiert wird. Alle weiteren expliziten Mengen müssen/können also mit Hilfe der anderen Axiome aus der leeren Menge konstruiert werden (siehe \ref{def-natural-numbers} und Definitionen/Bemerkungen danach).
        \item (ZF6): Das Potenzmengenaxiom garantiert auch, dass für eine Menge $A$ jede mögliche Teilmenge von $A$ existiert.
        \item (ZF7): Das Unendlichkeitsaxiom ist eng verknüpft mit der Definition der natürlichen Zahlen \ref{def-natural-numbers} und dem Prinzip der natürlichen Induktion \ref{theorem-natural-induction}.
        \item (ZF9): Damit wird zum Beispiel ausgeschlossen, dass Mengen sich nicht selbst enthalten können. Zu einer Menge $A$ betrachte zum Beispiel $B = \{A\}$, nach (ZF9) muss es ein $x \in B$ geben mit $x \cap B = \e$. Es gibt aber nur $x = A$, damit ist $A \cap \{A\} = \e$, also ist nach \ref{def-subset-union-cap} $A \notin A$.
    \end{itemize}
\end{remark}
\footnotetext{Zum Beispiel würde daraus auch sofort das Leermengenaxiom folgen, da $\e = \{x | x \neq x\}$.}
...
Man bemerke, dass die Paare $(a, b)$ geordnet sind, d.h. wenn $A$, $B$ zwei Elemente $x \neq y$ gemeinsam haben, dann ist $(x, y) \neq (y, x)$. Dafür reichen übliche Mengentupel $\{a, b\}$ nicht aus, da $\{a, b\} = \{b, a\}$. Die Existenz von geordneten Paaren ist z.B. durch (ZF2) mittels einer Identifikation wie $(a, b) = \{a, \{a, b\}\}$ begründet.
...
\begin{lemma}[Regeln von De Morgan]
    ...
    \end{enumerate}
\end{lemma}
Wir sehen nun unseren ersten kleineren Beweis. Die Beweismethode, die wir hier verwenden, nennt man \underline{Nachrechnen}. Damit meint man in der Mathematik das wiederholte Anwenden von Axiomen, Definitionen, und bereits gezeigten Aussagen, ohne dabei größere Ideen zu brauchen oder auf kompliziertere Details achten zu müssen.
\begin{proof}
    \begin{enumerate}
...
\begin{remark}
    Eine Wohlordnung kann man als eine \emph{kanonische} (d.h. für die menschliche Intuition die natürliche Wahl) Reihenfolge von Elementen in $X$ verstehen: Für die Teilmenge $X \subset X$ gibt (W) einem ein Minimum $w_1$, dieses nimmt man als erstes Element in der Reihenfolge. Dann gibt (W) einem für $X \backslash \{w_1\} \subset X$ wieder ein Minimum $w_2$, dann für $X \backslash \{w_1, w_2\}$ ein Minimum $w_3$, und so weiter. Durch Induktion\footnote{Endliche, natürliche, oder transfinite Induktion, je nach Kardinalität von $X$, siehe \ref{def-cardinality}, \ref{theorem-natural-induction}, \ref{remark-transfinite-induction}.} kann man alle Elemente von $X$ relativ zueinander ordnen: $w_1, w_2, w_3, ...$

    Die Existenz einer Wohlordnung ist für endliche Posets trivial: Man zählt die Elemente einfach in einer beliebigen Reihenfolge auf, und definiert sich entsprechend $x < y \iff x$ in der Aufzählung vor $y$ steht. Die gewählte Aufzählung ist dann auch schon die Wohlordnung. Für gewisse unendliche Mengen kann man auch eine Wohlordnung angeben, z.B. für $\N$ ist die Aufzählung $\{0, 1, 2, 3, ...)\}$ eine mögliche Wohlordnung mit der Wohlordnugnsrelation $<$. $Z$ und $Q$ können auch wohlgeordnet werden, siehe dazu den Beweis von \ref{theorem-cantor-1}, allerdings ist $<$ keine Wohlordnungsrelation mehr, weil $\Z$ und $\Q$ kein Minimum bezüglich $<$ haben. Die Existenz einer Wohlordnung für alle unendlichen Mengen, insbesondere für überabzählbare Mengen (\ref{def-cardinality}), benötigt ein zusätzliches Axiom (\ref{chapter-tenth-axiom}).
\end{remark}
...
\begin{example}
    \begin{enumerate}[label=(\arabic*)]
        ...
        $\text{Vorlesung}, \text{trinken}\}$ definieren wir zwei Funktionen:
        ...
        \end{itemize}
        Für uns Menschen sieht die Funktion $f$ nach einer sinnvollen Zuordnung aus, die Funktion $g$ dagegen völlig unsinnig. Aus mathematischer Sicht sind allerdings beide Funktionen absolut gleichwertig, nämlich wohldefiniert und bijektiv (siehe \ref{def-injective-surjective-bijective}).\\
...
\begin{definition}[injektiv, surjektiv, bijektiv]
    \label{def-injective-surjective-bijective}
    ...
    \begin{enumerate}
        ...
        Manchmal lässt sich Injektivität leichter durch die Kontraposition der Aussage zeigen: $\forall x, y \in X: f(x) = f(y) \implies x = y$.
...
\begin{remark}
    ...
    Mathematiker sind sich nicht einig, ob die $0$ ein Element der natürlichen Zahlen sein sollte. Manche Autoren, so auch wir oder Gurau, definieren $\N$ als natürliche Zahlen mit $0$ und schreiben $\N^*$ für die natürlichen Zahlen ohne $0$. Andere Autoren definieren $\N$ als natürliche Zahlen ohne $0$ und schreiben $\N_0$ für die natürlichen Zahlen mit $0$. Zwar ist $\N^*$ immer ohne $0$ und $\N_0$ immer mit $0$, aber wofür $\N$ steht, muss der Leser bei jeder neuen Lektüre selbst nachschauen, oder aus dem Kontext erschließen.
\end{remark}
...
\begin{theorem}[Vollständige Induktion]
    \label{theorem-natural-induction}
...
\begin{remark}
    \label{remark-transfinite-induction}
    Transfinite Induktion: für alle total geordneten Mengen
\end{remark}
...
\begin{definition}[Ganze Zahlen]
    Wir definieren die Menge der \underline{ganzen Zahlen} $\Z$ als den kleinsten kommutativen Ring mit Eins, der $\N$ mit der entsprechenden Addition und Multiplikation enthält.
\end{definition}
...
\begin{definition}[Rationale Zahlen]
    Wir definieren die Menge der \underline{rationalen Zahlen} $\Q$ als den kleinsten Körper, der $\Z$ mit der entsprechenden Addition und Multiplikation enthält.
\end{definition}
...
\begin{definition}[Reelle Zahlen]
    Wir definieren die Menge der \underline{reellen Zahlen} $\R$ als den vollständigen Körper, der $\Q$ mit der entsprechenden Ordnungsrelation $\leq$ enthält.
\end{definition}
...
\end{theorem}
\begin{proof}
    Führen wir hier nicht aus. Der motivierte Leser kann den Beweis in \cite{Amann-Escher-1}, Seiten 32-37 und 90-101, nachlesen.
\end{proof}
...
\begin{remark}
    Vorsicht: Unendlich, wie es in diesem Kontext definiert ist, ist keine Zahl! Man kann $\infty$ nicht in sinnvoller Weise zu $\R$ hinzufügen:

    $\infty + 1 = \infty + 0 \overset{\text{Kürzen}}{\underset{\ref{lem-rechenregeln-gruppe}}{\implies}} 0 = 1$, analoges für $\infty \cdot 2 = \infty \cdot 1 \implies 2 = 1$

    Unendlich ist auch nicht die multiplikative Inverse zu $0$:
    
    Angenommen, es gibt $\frac{1}{0} = \infty \implies 0 \overset{\text{Def. Null}}{=}\frac{1}{0} \cdot 0 \overset{\text{Def. Inverse}}{=} 1$

    Bottom line: $\infty$ ist keine Zahl, und $\frac{1}{0}$ existiert immer noch nicht.
\end{remark}
...
\begin{proof}
    ...
    \begin{itemize}[label=]
        ...
        Dass $|X|$ also kleiner als $|\Pot(X)|$ sein muss, ist intuitiv klar. Formal zeigt man das mithilfe der Einbettung $\iota: X \arr \Pot(X)$, $\iota(x) = \{x\}$:
...
\begin{proof}
    ...
    \begin{enumerate}[label=Fall \arabic*:]
        \item $w \geq 0$. Dann ist $n = 2w \in \N$ mit, da $n$ gerade, $f(n) = \frac{2 w}{2} = w$.
        \item $w < 0$. Da $-2w \geq 2$ ist $n = -2w - 1 \in \N$  mit, da $n$ ungerade, $f(n) = - \frac{-2w - 1 + 1}{2} = w$.
    \end{enumerate}
    ...
    Das Schema ist offensichtlich injektiv. Weil jede Diagonale mit einem endlichen Anfangspunkt endlich ist, erreicht man jede endliche rationale Zahl nach endlich vielen Schritten. Deswegen muss das Schema auch surjektiv sein.

    \underline{Schritt 3}: $\Q$ ist abzählbar.

    Folgt durch eine Kette von Bijektionen:
    
    $\N \quad \cong \quad \Z \quad = \quad (-\N^*) \cup \N \quad \cong \quad \Q_{< 0} \cup \Q_{\geq 0} \quad = \quad \Q$.
...
\begin{definition}[Komplexe Zahlen]
    Wir definieren die Menge der \underline{komplexen Zahlen} $\C$ als den kleinsten Körper, in dem die Gleichung $x^2 = -1$ eine Lösung hat, und der $\R$ enthält.
\end{definition}

\begin{corollary}["für Marco"]
    Diese Definition ist logisch und ergibt Sinn.
\end{corollary}
\begin{proof}
    Klar.
\end{proof}
...
\begin{definition}[Eigenschaften von komplexen Zahlen]
    ...
    \begin{enumerate}
        ...
        \item $|z| = \sqrt{x^2 + y^2}$ den \underline{Betrag} von $z$
    \end{enumerate}
\end{definition}
...
Intuitiv kann man die natürlichen Zahlen fordern, indem man $0$ als "nichts" definiert, $1$ als "eines", und von da an jede kommende Zahl als "die Zahl davor $+ 1$"\footnote{Formal machen das die Peano-Axiome.}. Mathematisch wäre streng genommen nichts falsch damit, und für ein mathematisches Verständnis ist die formale Definition auch nicht entscheidend. Der ästhetische Mangel ist allerdings, dass man dazu ein zusätzliches Axiom bräuchte, was aber nicht nötig ist, da man die bereits bekannte Struktur der natürlichen Zahlen mit den bereits bekannten Axiomen konstruieren kann. Das macht man genauso wie beschrieben, d.h. man fängt mit "nichts" an und geht jeweils Schritt für Schritt weiter "hoch". Dafür verwendet man die Existenz der leeren Menge (ZF5) sowie ein bisschen Trixerei mit Mengenklammern.
