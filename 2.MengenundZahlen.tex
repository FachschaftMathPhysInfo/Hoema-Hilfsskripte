...
\begin{axiom}[Zermelo-Fraenkel-Axiome] %[vgl Axiom 2.1]
    ...
    \begin{enumerate}[label=(ZF\arabic*)]
        ...
        \item \underline{Vereinigungsaxiom}: Wenn $A$ eine Menge ist, die aus Mengen besteht, dann ist $\bigcup A = \{x | x \in B \land D \in A\}$ eine Menge.
...
\begin{theorem}[$\Q$ liegt dicht in $\R$]
    \label{theorem-q-liegt-dicht-in-r}
\end{theorem}

\section{Komplexe Zahlen}

Die komplexen Zahlen stammen historisch aus der Realisierung, dass die Lösung für gewisse kubische Gleichungen (d.h. Gleichungen der Form $a \cdot x^3 + b \cdot x^2 + c \cdot x + d = 0$) zwar reelle Lösungen haben, aber nur berechnet werden können, wenn man die Existenz von Wurzeln aus negativen Zahlen zulässt, obgleich besagte Wurzeln in der Lösung nicht vorkommen. Das führt zur Einführung der imaginären Einheit, die ihren Namen daher hat, dass sie die Seitenlänge eines Quadrates mit negativem Flächeninhalt beschreibt, was empirisch nicht möglich scheint.
